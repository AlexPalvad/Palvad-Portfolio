import { ArrowUpRight } from "lucide-react";
import { useLang } from "@/lib/i18n";

const Projects = () => {
  const { t } = useLang();

  const projects = [
    {
      name: "Palvad.dk",
      description: t("projects.palvad.desc"),
      tags: ["Lovable", "Supabase", "Make.com"],
      url: "https://palvad.dk",
      status: "Active",
    },
    {
      name: "Palvad Clubs",
      description: t("projects.clubs.desc"),
      tags: ["Lovable", "Supabase", "Capacitor", "Xcode"],
      url: "https://clubs.palvad.dk",
      status: "In Development",
    },
    {
      name: "Presslab.dk",
      description: "Digital platform for a brand and merchandise company. Product presentation, brand communication, and marketing materials.",
      tags: ["Lovable"],
      url: "https://presslab.dk",
      status: "Active",
    },
    {
      name: "Skomar Akademiet",
      description: "Online training platform focused on the Danish game 'Skomar'. Structured teaching material and learning progression.",
      tags: ["Lovable"],
      url: "https://skomarakademiet.dk",
      status: "Active",
    },
    {
      name: "GearTjek.dk",
      description: "Review and comparison website for security equipment and home technology. Product reviews, comparisons, and guides.",
      tags: ["Lovable"],
      url: "https://geartjek.dk",
      status: "Active",
    },
    {
      name: "HairDesign by Enis",
      description: "Professional website for a hair salon in Vangede, Gentofte. Service overview, pricing, gallery, and contact information.",
      tags: ["Lovable"],
      url: "https://www.hairdesignbyenis.dk/",
      status: "Active",
    },
    {
      name: "Palvad Tools",
      description: t("projects.tools.desc"),
      tags: ["Planned"],
    },
    {
      name: "Other Projects",
      description: t("projects.other.desc"),
      tags: ["Make.com", "ChatGPT", "Claude"],
      status: "Experimental",
    },
  ];

  return (
    <section id="projects" className="container py-16 border-t border-border">
      <h2 className="font-display font-bold text-2xl mb-6 tracking-tight">{t("projects.title")}</h2>
      <div className="grid gap-4 md:grid-cols-2">
        {projects.map((p) => (
          <div
            key={p.name}
            className="border border-border rounded-lg p-5 bg-card hover:border-primary/30 transition-colors duration-150"
          >
            <div className="flex items-start justify-between mb-2">
              <h3 className="font-display font-semibold text-foreground">
                {p.url ? (
                  <a
                    href={p.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 hover:text-primary transition-colors duration-150"
                  >
                    {p.name} <ArrowUpRight className="w-3.5 h-3.5" />
                  </a>
                ) : (
                  p.name
                )}
              </h3>
              {p.status && (
                <span
                  className={`text-xs font-mono px-2 py-0.5 rounded-sm ${
                    p.status === "Active"
                      ? "bg-accent/15 text-accent"
                      : "bg-secondary text-muted-foreground"
                  }`}
                >
                  {p.status}
                </span>
              )}
            </div>
            <p className="text-sm text-muted-foreground mb-3">{p.description}</p>
            <div className="flex flex-wrap gap-1.5">
              {p.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-xs font-mono px-2 py-0.5 rounded-sm bg-secondary text-muted-foreground"
                >
                  {tag}
                </span>
              ))}
            </div>
          </div>
        ))}
      </div>
    </section>
  );
};

export default Projects;
